...
<body>
	<br>
	<form id="Form1" name="Form1" action="${pageContext.request.contextPath}/orders/list.html" method="post">
						...
						<table cellpadding="0" cellspacing="0" border="0" width="100%">
							<tr>
								...
								<td class="ta_01" bgColor="#ffffff"><input type="text"
									name="id" size="15"   id="Form1_userName1" class="bg" />
								</td>
								<td height="22" align="center" bgColor="#f5fafe" class="ta_01">
									收件人：</td>
								<td class="ta_01" bgColor="#ffffff"><input type="text"
									name="recevice_Name" size="15" id="Form1_userName"
									class="bg" /></td>
							</tr>
				...
				<tr>
					<td class="ta_01" align="center" bgColor="#f5fafe">
									...
									<c:forEach items="${pageInfo.list }" var="order" varStatus="vs">
										<tr onmouseover="this.style.backgroundColor = 'white'"
											...
											<td style="CURSOR: hand; HEIGHT: 22px" align="center"
												width="20%" id="order_Id" >${order.id }</td>
											<td style="CURSOR: hand; HEIGHT: 22px" align="center"
												width="10%">${order.receiver_name }</td>
											<td style="CURSOR: hand; HEIGHT: 22px" align="center"
												width="15%">${order.receiver_address }</td>
											<td style="CURSOR: hand; HEIGHT: 22px" align="center"
												width="10%">${order.receiver_phone }</td>
											<td style="CURSOR: hand; HEIGHT: 22px" align="center">
												${order.money}</td>
											<td width="8%" align="center">${order.user.name}</td>
											<td width="10%" align="center">${order.status==0?"未支付":"已支付" }</td>
			
											<td align="center" style="HEIGHT: 22px"><a
												href="${pageContext.request.contextPath}/order_items/getInfo.html?order_Id=${order.id }">
													<img
											...
											<td align="center" style="HEIGHT: 22px">
													<a
														...
														width="16" height="16" border="0" style="CURSOR: hand" onclick="del(${order.id})">
													</a>
											</td>
						...
						<a href="/orders/list.html?
							id=${id}&recevice_Name=${recevice_Name}
							&pageNo=${pageInfo.prePage}">上一页</a>

						<c:forEach begin="${pageInfo.navigateFirstPage}" end="${pageInfo.navigateLastPage}" var="pn" >
							<a href="/orders/list.html?
								id=${id}&recevice_Name=${recevice_Name}
							&userId=${param.userId}&pageNo=${pn}">
						...
						<a href="/orders/list.html?
							id=${id}&recevice_Name=${recevice_Name}
						&pageNo=${pageInfo.nextPage}">下一页</a>
					</td>
				</tr>
			</TBODY>
		</table>
	...
	<script type="text/javascript">
        function del(id){
            ...
            if(confirm("确定删除吗？")){
                $.ajax({
                    type:"post",
                    url:"/orders/del.html?id="+id,
                    success:function(result){
                        alert(result.msg);
                        window.location.href='${pageContext.request.contextPath}/orders/list.html';
                    }
                });
            }
        }
