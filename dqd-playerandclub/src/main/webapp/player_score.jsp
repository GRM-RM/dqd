...
<div id="main" class="cf">
    ...
    <div id="stat_detail">
        ...
        <table class="list_1" id="table">
            ...
            <tr>
                ...
                <th class="team">球队</th>
                <th class="stat">进球数</th>
            </tr>

            <c:forEach items="${page.list}" var="player" varStatus="vs">
                <tr>
                    <td class="rank">${vs.count}</td>
                    <td class="player">
                        ${player.name}
                    </td>
                    <td class="team">
                ...
                <td colspan="4">
                    <c:if test="${page.pages>1}">
                        ...
                        <c:if test="${page.hasPreviousPage}">
                            <a href="/player/list?mark=1&country=${country}&pageIndex=${page.pageNum-1}">上一页</a>
                        </c:if>

                        <c:forEach items="${page.navigatepageNums}" var="np">
                            <c:choose>
                                <c:when test="${page.pageNum==np}">
                                    <a style="font-size: 20px" href="/player/list?mark=1&country=${country}&pageIndex=${np}">${np}</a>
                                </c:when>
                                <c:otherwise>
                                    <a href="/player/list?mark=1&country=${country}&pageIndex=${np}">${np}</a>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>

                        <c:if test="${page.hasNextPage}">
                            <a href="/player/list?mark=1&country=${country}&pageIndex=${page.pageNum+1}">下一页</a>
                        </c:if>
    ...
    <script type="text/javascript">
        function go() {
            ...
            window.location.href="/player/list?mark=1&country=${country}&pageIndex="+num;
        }
    </script>
